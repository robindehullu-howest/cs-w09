...
{
    [FunctionName("StartEventTrigger")]
    public async Task<IActionResult> StartEvent(
        [HttpTrigger(AuthorizationLevel.Anonymous, "put", Route = "event/{id}/start")] HttpRequest req, string id,
        ILogger log)
    {
        if (!string.IsNullOrEmpty(id))
        {
            ...
            while (iterator.HasMoreResults)
            {
                ...
                if (eventData != null && eventData.Start == DateTime.MinValue)
                {
                    eventData.Start = DateTime.Now;
                    ...
                    return new BadRequestObjectResult("Event already started.");
            }
        }
